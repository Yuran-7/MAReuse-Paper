...
Following the evaluation protocol of KVCOMM~\citep{DBLP:journals/corr/abs-2510-12872}, we randomly sample evaluation instances from the original benchmark datasets to keep the exhaustive multi-agent evaluation across methods and workflow widths computationally manageable. Specifically, we sample 659 examples from the GSM8K test set, 153 questions from the MMLU test set, and 161 tasks from the HumanEval test set. Sampling is performed once before running the experiments and without replacement. We use the same fixed subset and evaluation order for every method and workflow width, so all reported comparisons are paired on identical inputs and no method-specific resampling is performed.

\subsection{Repetition and Measurement Variability}

All methods use greedy decoding with temperature 0, fixed prompts, and a fixed evaluation order. Across repeated executions, we empirically observe identical predictions and accuracy for each configuration, resulting in no observed run-to-run accuracy variation under our setup. This observation is empirical rather than a general guarantee of bitwise determinism, as discussed in Section~C.4.

Each runtime configuration, defined by a method, benchmark, and workflow width, is measured over five repeated runs, with method-specific caches and indices reset before each run. For each configuration, we compute the mean and a 95\% confidence interval using Student's $t$-distribution. The evaluation contains 72 TTFT configurations, covering six methods across three benchmarks and four workflow widths ($6\times3\times4$), and 60 decoding-throughput configurations, covering five methods under the same benchmarks and workflow widths ($5\times3\times4$). The main result tables retain the mean values without displaying a separate interval in every entry. Across the TTFT configurations, the median and maximum relative 95\% confidence-interval half-widths are 1.38\% and 2.01\%, respectively. Across the decoding-throughput configurations, the corresponding values are 1.93\% and 2.82\%, respectively.
...
\begin{itemize}
    ...
    \item \textbf{KVCOMM}~\citep{DBLP:journals/corr/abs-2510-12872}: a multi-agent KV communication framework that estimates prefix-conditioned KV offsets through similarity retrieval from role-specific anchor pools. We follow the default configuration of the original method and use an anchor pool size of 20 per agent role.
...
\begin{itemize}
    ...
    \item \textbf{RACER}~\citep{DBLP:conf/acl/ZhangLZWZ26}: a retrieval-enhanced speculative decoding method that combines logits-tree candidates with Aho-Corasick matching. Following the original paper, we use a 61-token draft budget and an Aho-Corasick automaton budget of 10,000 nodes.
...
We also evaluate candidate filter layers on HumanEval under 2--5 agent workflows, using the same 20\% recomputation budget as the main evaluation. Figure~\ref{fig:supp-humaneval-filter-layer-ablation} shows a less pronounced but still non-monotonic pattern on HumanEval, with average Pass@1 peaking at layer 7 (86.96\%). Together, the two task-level evaluations support using layer 6 for Llama-3.1-8B-Instruct on MMLU and layer 7 for Qwen2.5-Coder-7B-Instruct on HumanEval, consistent with choosing the filter layer through an offline model- and task-specific evaluation rather than fixing it by depth alone.
...
We report TTFT and decoding throughput separately rather than presenting a single end-to-end latency number. In the evaluated reasoning and code-generation workflows, each agent produces a multi-token response autoregressively, making decoding the principal serial component and typically the larger contributor to total latency. Nevertheless, accelerating prefill remains practically important. First, it directly reduces the response-initiation latency of every downstream agent. Second, prefill cost grows as downstream prompts accumulate outputs from upstream agents, so redundant prompt processing becomes increasingly expensive in wider or longer workflows. Third, shorter prefills release GPU resources earlier and can improve responsiveness and scheduling flexibility in a serving system. The exact end-to-end contribution of each stage depends on prompt and output lengths, workflow topology, batching, and the serving policy. A unified evaluation under broader serving workloads is left for future work.

\paragraph{Accuracy under Joint Reuse.}
When prefill reuse and collaborative speculative decoding are enabled simultaneously, MAReuse still exhibits accuracy degradation in some settings, particularly on MMLU. Compared with the unoptimized pipeline, the combined configuration drops from 84.31\% to 80.39\% with three agents, from 80.39\% to 77.37\% with four agents, and from 82.35\% to 76.47\% with five agents. The largest observed drop is therefore 5.88 percentage points. These results indicate that MMLU is more sensitive to the accumulated numerical and representation changes introduced by the joint reuse pipeline than GSM8K and HumanEval. Thus, MAReuse should be interpreted as providing a workload-dependent quality--efficiency trade-off rather than universally preserving task accuracy. Future work will investigate confidence-aware reuse policies that selectively fall back to full prefill or standard autoregressive decoding for sensitive inputs.
