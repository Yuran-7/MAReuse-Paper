...
This section presents implementation details of MAReuse-D. We first describe online construction of the local suffix automaton, then define its runtime retrieval and update operations. We next introduce the role-aware global index. Finally, we show how local retrieval, global retrieval, and the logits-tree fallback are composed during decoding, followed by their memory costs.
...
The main reason we use a SAM is its ability to adapt the matched context length to each query. Responses from different agents within the same sample often share only partial local contexts. By tracking the longest suffix present in the history and following suffix links when backoff is needed, the SAM preserves the longest valid match, whereas global matching relies on a small set of fixed anchor lengths and may miss reusable continuations.
...
tokens \texttt{ABC} as the continuation. Algorithm~\ref{alg:sam-build}
summarizes the online extension procedure used to construct the SAM one
token at a time.
